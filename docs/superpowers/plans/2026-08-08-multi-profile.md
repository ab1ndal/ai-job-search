...
- Scope addition folded into Task 11: `README.md`, `SETUP.md`, and `AGENTS.md` still describe root-relative candidate paths (tracker CSV, `documents/`, generated CV and cover-letter locations). Stale docs are bugs; Task 11 converts them and brings them under the audit.
- Scope addition folded into Task 10: several specs still point at root `CLAUDE.md` for CANDIDATE DATA (`apply.md:89` CV language, `apply.md:137` Candidate Profile section, `job-scraper/SKILL.md:129` Languages table, `search-queries.md:11,78`, `04-job-evaluation.md:37`). Task 10 empties `CLAUDE.md` of candidate data, so these must repoint to `profiles/<name>/skills/01-candidate-profile.md` or `profiles/<name>/PROFILE.md`. References to CLAUDE.md's **verification checklist** (`apply.md:299,302`) are correct as-is and stay.
